...
def add_task():
    ...
    if task and deadline:
        try:
            ...
            task_var.set('')
            deadline_var.set('')
            save_tasks_to_csv(task, deadline)  # Save task to CSV
        except ValueError:
...
def update_task():
    ...
    if selected_item:
        ...
        task_var.set('')
        deadline_var.set('')
        save_tasks_to_csv(task_var.get(), deadline_var.get())  # Update task in CSV
    else:
...
def delete_task():
    ...
    if selected_item:
        ...
        save_tasks_to_csv()  # Update CSV after deletion
    else:
...
def save_tasks_to_csv(task='', deadline=''):
